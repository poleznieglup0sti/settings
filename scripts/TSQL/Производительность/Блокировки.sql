...
DECLARE @ID int = DB_ID('DB1CV8-MNC-UPP-2013');
...
SELECT
  ...
  WHERE t.session_id > 50 AND t.wait_duration_ms > @Wait_Duration_ms AND l.resource_database_id = @ID;
...
SELECT DISTINCT
  ...
  WHERE r.session_id > 50 AND r.session_id <> @@spid
    AND s.[program_name] NOT LIKE 'SQL Server Profiler%' AND r.database_id = @ID
