...
cat rphost_*/*1217.log | awk -F\- '/^[0-9]+:[0-9]+.[0-9]+-/{print $2}' | awk -F\, '{sum[$2]+=$1;} END {for (event in sum) print sum[event]" - "event;}' | sort -rnb > 1.txt
...
grep -v -E "(РТИ_ШаблоныЗаполненияРеквизитов|ЗадачаИсполнителя)" |
sed -r 's/\x27(\\\\.{,7}){1,15}\x27::(bytea|mvarchar|timestamp)/@P/g;s/\t/ /g;s/ +/ /g' | 
gawk -F '.S=' '{Array[$3]+=$1; Array2[$3]+=1} END {for(i in Array) print Array[i] ", " Array2[i] ":" i}' |   
sort -rnb | head -n 10 > 1.txt
